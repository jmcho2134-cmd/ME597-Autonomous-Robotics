...
USE_COST_ASTAR     = True
UNKNOWN_COST       = 0.0     # unknown cell penalty (can be 0 if the map is fully known)
OBSTACLE_COST_GAIN = 10.0    # how strongly to avoid being close to walls
OBSTACLE_SAFE_DIST = 0.65     # start increasing penalty within this distance [m]

# ===== LaserScan-based dynamic obstacles =====
LASER_FRONT_DEG       = 45.0    # forward +/- angular range [deg]
LASER_OBS_DIST        = 1.0     # treat hits within this distance as obstacles [m]
PATH_OBS_HIT_DIST     = 0.8     # consider it "on the path" if within this distance to the path [m]
DYN_OBS_INFLATION_RAD = 0.15    # dynamic obstacle inflation radius [m]
...
# ===== RRT* parameters for subpath planning =====
RRT_MAX_ITERS        = 1500
RRT_STEP_SIZE        = 0.20   # [m]
RRT_GOAL_SAMPLE_RATE = 0.15   # probability of sampling the goal directly (15%)
RRT_NEAR_RADIUS      = 0.35   # [m] rewiring radius
RRT_GOAL_TOL         = 0.35   # [m] consider goal reached within this distance
RRT_BOUND_MARGIN     = 0.8    # [m] expand the sampling box around start/goal by this margin
...
class MapProcessor:
    ...
    def update_from_msg(self, msg: OccupancyGrid, inflation_radius_m: float = wall_buffer_m):
        ...
        # Compute distance map + collision inflation based on the static map
    ...
    def _inflate_grid(self, data: List[int], w: int, h: int, inflation_radius_m: float) -> List[int]:
        ...
        # 2) Collision inflation: unknown + near-obstacle cells
        cells = int(math.ceil(inflation_radius_m / max(self.resolution, 1e-6)))
        inflated = [0] * (w * h)
        for i, d in enumerate(dist):
            v = data[i]
            if v < 0:
                # Unknown regions are always blocked
                inflated[i] = 1
            elif d <= cells:
                inflated[i] = 1
            else:
        ...
        # Also block dynamic obstacle layer (if present)
    ...
    # ----- Raycast to obstacle along a beam -----
...
class AStar:
    ...
    def _cell_cost(self, idx: int) -> float:
        ...
        # Increase cost when closer to walls
...
class Task2(Node):
    ...
    # ----- RRT* subpath planner (world coordinates) -----
    def _rrt_star_subpath(self,
                          start_xy: Tuple[float, float],
                          goal_xy: Tuple[float, float]) -> Optional[List[Tuple[float, float]]]:
        
        if not self.map_ready or self.map_proc.width == 0 or self.map_proc.height == 0:
            return None
        ...
        # Set sampling bounding box (start/goal neighborhood + margin)
        map_x_min = mp.origin_x
        map_y_min = mp.origin_y
        map_x_max = mp.origin_x + mp.width * mp.resolution
        map_y_max = mp.origin_y + mp.height * mp.resolution

        xmin = max(min(sx, gx) - RRT_BOUND_MARGIN, map_x_min)
        xmax = min(max(sx, gx) + RRT_BOUND_MARGIN, map_x_max)
        ymin = max(min(sy, gy) - RRT_BOUND_MARGIN, map_y_min)
        ymax = min(max(sy, gy) + RRT_BOUND_MARGIN, map_y_max)

        if xmin >= xmax or ymin >= ymax:
            return None

        def is_free_world(x: float, y: float) -> bool:
            gx, gy = mp.pose_to_grid(x, y)
            return mp.is_free(gx, gy)

        def is_segment_free(p: Tuple[float, float],
                            q: Tuple[float, float]) -> bool:
            px, py = p
            qx, qy = q
            dist = math.hypot(qx - px, qy - py)
            if dist < 1e-6:
                return is_free_world(px, py)
            # Sample densely (about half a cell resolution)
            step = max(mp.resolution * 0.5, 0.02)
            n_steps = int(dist / step)
            for i in range(n_steps + 1):
                t = i / max(n_steps, 1)
                x = px + (qx - px) * t
                y = py + (qy - py) * t
                if not is_free_world(x, y):
                    return False
            return True

        # Check start/goal are in free space
        if not is_free_world(sx, sy) or not is_free_world(gx, gy):
            return None

        # Node / parent / cost structures
        ...
        for it in range(RRT_MAX_ITERS):
            ...
            if random.random() < RRT_GOAL_SAMPLE_RATE:
                ...
                # Try a limited number of times until a free-space sample is found
                found = False
                for _ in range(30):
                    rx = random.uniform(xmin, xmax)
                    ry = random.uniform(ymin, ymax)
                    if is_free_world(rx, ry):
                        found = True
                        break
                if not found:
                    continue
            ...
            nx, ny = nodes[nearest_i]
            if nearest_d < 1e-6:
                continue

            # Steer: move toward sample by step size
            step = min(RRT_STEP_SIZE, nearest_d)
            theta = math.atan2(ry - ny, rx - nx)
            new_x = nx + step * math.cos(theta)
            new_y = ny + step * math.sin(theta)

            if not is_free_world(new_x, new_y):
                continue
            if not is_segment_free((nx, ny), (new_x, new_y)):
                continue

            # Choose best parent among near set
            new_cost = costs[nearest_i] + math.hypot(new_x - nx, new_y - ny)
            ...
            for i, (px, py) in enumerate(nodes):
                if i == nearest_i:
                    continue
                d = math.hypot(new_x - px, new_y - py)
                if d > RRT_NEAR_RADIUS:
                    continue
                if not is_segment_free((px, py), (new_x, new_y)):
                    continue
                cand_cost = costs[i] + d
            ...
            # Rewire
            for i, (px, py) in enumerate(nodes[:-1]):
                d = math.hypot(new_x - px, new_y - py)
                if d > RRT_NEAR_RADIUS:
                    continue
                if not is_segment_free((px, py), (new_x, new_y)):
                    continue
                cand_cost = best_cost + d
                if cand_cost < costs[i]:
                    costs[i] = cand_cost
                    parents[i] = new_idx

            # Goal reached check
            if math.hypot(new_x - gx, new_y - gy) <= RRT_GOAL_TOL and \
               is_segment_free((new_x, new_y), (gx, gy)):
                # Add goal node
                nodes.append((gx, gy))
                parents.append(new_idx)
                costs.append(best_cost + math.hypot(gx - new_x, gy - new_y))
                goal_idx = len(nodes) - 1
                break

        # If goal not reached in loop, try connecting the closest node to the goal
        if goal_idx is None:
            best_i = None
            best_d = 1e9
            for i, (px, py) in enumerate(nodes):
            ...
            if best_i is None:
                return None
            if best_d <= RRT_GOAL_TOL * 1.5 and is_segment_free(nodes[best_i], (gx, gy)):
                nodes.append((gx, gy))
                parents.append(best_i)
                costs.append(costs[best_i] + best_d)
                goal_idx = len(nodes) - 1
            else:
                return None
    ...
    # ----- Subpath replanning helper using RRT* -----
    def _replan_subpath_around_obstacle(self, obs_x: float, obs_y: float) -> bool:
        ...
        # 1) Find the path index closest to the obstacle (only after current target)
        min_d = 1e9
        obs_idx = -1
        ...
        # 2) Find two safe points far enough from the obstacle
        SAFE_CLEAR_RADIUS = 0.5  # subpath start/end must be at least this far from the obstacle
        ...
        # 3) Replan only that segment with RRT* (world coordinates)
        sub_path_xy = self._rrt_star_subpath(sub_start_xy, sub_goal_xy)
        if sub_path_xy is None or len(sub_path_xy) < 2:
            return False
        ...
        self.get_logger().info(
            f'RRT* subpath replanned around obstacle at ({obs_x:.2f}, {obs_y:.2f}) '
    ...
    def __scan_cbk(self, msg: LaserScan):
        ...
        for i, r in enumerate(msg.ranges):
            ...
            # 1-1) Ignore if too close to a static wall (cost-based wall avoidance region)
            ...
            # If the static grid already marks it as occupied (map mismatch, etc.), skip
            ...
            # 2) Check whether the hit is on/near the path (only check forward segment)
            on_path = False
            ...
            # 4) Try RRT* subpath replanning
            success = self._replan_subpath_around_obstacle(wx, wy)
            if not success:
                # If it fails, fall back to full replanning (full A* will run on next loop)
                self.last_goal_xy = None
                self.get_logger().warn(
                    'RRT* subpath replanning failed, will trigger full A* replan on next loop.'
                )
            break   # handle only one hit per callback, then exit
